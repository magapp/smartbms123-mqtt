...
const SmartBmsError SmartBmsReader::decodeBmsData(SmartBmsData *smartBmsData) const
{
	...
	smartBmsData->maxTemperatureAlarmActive_ = buffer[30] & 0b01000000;
        ...
        smartBmsData->currentCell_ = buffer[24];
	//smartBmsData->currentCellVoltage_ = this->decodeCellVoltage_(&buffer[28]);
	//smartBmsData->currentCellTemperature_ = this->decodeCellTemperature_(&buffer[26]);
	smartBmsData->currentCellVoltage_ = this->decodeCellVoltage_(&buffer[26]);
	smartBmsData->currentCellTemperature_ = this->decodeCellTemperature_(&buffer[28]);
